...
const MobileProjectsImproved = ({ active, setActive }) => {
...
const ProjectCard = ({
  ...
  // Improved center detection with intersection observer
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        // Check if element center is close to viewport center
        const distanceFromCenter = Math.abs(elementCenter - viewportCenter);
        const threshold = rootBounds.height * 0.2; // 20% tolerance for better detection

        const inCenter = distanceFromCenter < threshold && entry.isIntersecting;
  ...
  // Handle expansion when card is in center
  useEffect(() => {
    if (isInCenter && !isExpanded && !isScrolling) {
  ...
  }, [isInCenter, isExpanded, isScrolling, onExpand, project.id]);
  ...
  return (
    <motion.div
      ref={cardRef}
      className="relative bg-[#0a1920] rounded-2xl overflow-hidden shadow-lg border cursor-pointer"
...
export default MobileProjectsImproved;
